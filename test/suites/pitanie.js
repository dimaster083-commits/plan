...
(async () => {
  ...
  const r = await page.evaluate(() => {
    ...
    S.goal = '90'; const up = nutriFor('up1'); const gainR = paceRange();
    S.goal = '70'; const dn = nutriFor('up1'); const lossR = paceRange();
    S.goal = '80'; const keep = nutriFor('up1');
  ...
  chk(Math.abs(r.up - Math.round(r.m80 * 1.12 / 10) * 10) <= 10, 'набор: +12 % к поддержанию', r.up + '');
  chk(Math.abs(r.dn - Math.round(r.m80 * 0.8 / 10) * 10) <= 10, 'снижение: −20 %', r.dn + '');
  chk(Math.abs(r.keep - Math.round(r.m80 / 10) * 10) <= 10, 'удержание: ровно поддержание', r.keep + '');
