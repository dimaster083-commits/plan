/* «ТЕМП» в разборе. Коридор набора — 0,25–0,5 % массы в неделю (Iraki 2019,
   Helms): при 80 кг это 0,20–0,40 кг. Раньше стоял жёсткий 0,3–0,5 кг, а
   нормой считалось всё от 0,1 до 0,7. И при перелёте цели он всё ещё
...
(async()=>{
  ...
  const b2=await tempo(80,80.6,95);  // 0,30 кг/нед
  chk(b2&&/в коридоре 0,2/.test(b2.text),'2. 0,30 кг/нед — в коридоре',b2?b2.text:'нет блока');
  const c=await tempo(80,81.0,95);   // 0,50 кг/нед
  chk(c&&/чуть быстрее коридора/.test(c.text),'3. 0,50 кг/нед — чуть быстрее коридора',c?c.text:'нет блока');
  ...
  const e=await tempo(95,95.5,95);   // цель чуть позади, движение вперёд в коридоре
