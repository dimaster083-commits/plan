/* Шкала «за всё время» и подпись недели плана.
   Шкала — логарифмическая до 1000 т, одна на все годы: при шкале до
   ближайшей отметки ведущая полоса всегда стояла почти залитой.
...
(async()=>{
  ...
  // 1-3. шкала одна на все годы — до 1000 т, логарифмическая: 8,8 т — середина
  // пути, а не почти залитая полоса, как при шкале до ближайшей отметки
  const a = await шкала([2.0,0.9,0.8,8.8,1.2,0]);
  chk(/1000 т/.test(a.потолок),'1. потолок шкалы — 1000 т',a.потолок);
  const ноги=a.строки.find(x=>x.имя==='Ноги');
  chk(ноги.ширина>40&&ноги.ширина<60,'2. 8,8 т — около середины, полоса не переполнена',
      ноги.имя+' '+ноги.число+' → '+ноги.ширина+'%');
  const спина=a.строки.find(x=>x.имя==='Спина');
  chk(Math.abs(спина.ширина-Math.log10(2/0.1)/4*100)<0.2,'3. деление шкалы — вдесятеро: 2 т на своём месте',
      спина.имя+' '+спина.число+' → '+спина.ширина+'%');

  // 4-5. рост не двигает шкалу, тысяча тонн — край, больше — упирается
  const c = await шкала([2.0,0.9,0.8,12,1.2,0]);
  chk(/1000 т/.test(c.потолок) && c.строки.find(x=>x.имя==='Спина').ширина===спина.ширина,
      '4. другая группа выросла — шкала и чужие полосы на месте',c.потолок);
  const e = await шкала([1000,2500,0,0,0,0]);
  chk(e.строки[0].ширина===100 && e.строки[1].ширина===100 && /2,5 тыс\. т/.test(e.строки[1].число),
      '5. 1000 т — край; больше — тоже край, число в тысячах тонн',e.строки.slice(0,2).map(x=>x.число+' '+x.ширина+'%').join(', '));
