...
(async()=>{
  ...
  // 4-5. дошёл до отметки — шкала переезжает на следующую, но мелким шагом:
  // прежний редкий ряд ронял ведущую группу с полной полосы до сорока процентов
  const c = await шкала([2.0,0.9,0.8,12,1.2,0]);
  chk(/12,5 т/.test(c.потолок),'4. перешагнул отметку — шкала переехала на следующую',c.потолок);
  const ноги2=c.строки.find(x=>x.имя==='Ноги');
  chk(ноги2.ширина>=60&&ноги2.ширина<100,'5. ведущая группа не схлопывается при переезде шкалы',ноги2.ширина+'%');
