...
(async()=>{
  ...
  // 7-9. подпись показывает посещаемость и календарное правило фазы
  ...
  chk(w.план===2,'7. план перешёл на календарную неделю','неделя плана '+w.план);
  chk(/Закрыто на этой неделе: 0 из 4/.test(w.текст),'8. подпись отделяет посещаемость текущей недели',w.текст.slice(0,200));
  chk(/фаза меняется по календарю каждый понедельник/.test(w.текст),'9. и объясняет календарное правило',
      /по календарю/.test(w.текст)?'объясняет':'молчит');

  // 10. добрали прошлую неделю — фаза остаётся календарной
  ...
  chk(w2.план===2,'10. прошлые отметки не меняют текущую фазу','неделя плана '+w2.план);
  chk(/фаза меняется по календарю каждый понедельник/.test(w2.текст),'11. подпись не обещает повтор фазы',
