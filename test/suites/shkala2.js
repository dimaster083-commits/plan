/* Шкала «за всё время» и подпись недели плана.
   Шкала — прямая, потолок в тысячах тонн: при шкале до ближайшей
   отметки ведущая полоса всегда стояла почти залитой.
...
(async()=>{
  ...
  // 1-3. шкала прямая, потолок в тысячах тонн: полоса заполняется медленно
  const a = await шкала([2.0,0.9,0.8,8.8,1.2,0]);
  chk(/1 тыс\. т/.test(a.потолок),'1. потолок шкалы — тысяча тонн',a.потолок);
  const ноги=a.строки.find(x=>x.имя==='Ноги');
  chk(Math.abs(ноги.ширина-0.88)<0.01,'2. 8,8 т — 0,88 % полосы: заполняется медленно',
      ноги.имя+' '+ноги.число+' → '+ноги.ширина+'%');
  const спина=a.строки.find(x=>x.имя==='Спина');
  chk(Math.abs(спина.ширина-0.2)<0.01,'3. шкала прямая: 2 т — 0,2 %',спина.имя+' '+спина.число+' → '+спина.ширина+'%');

  // 4-5. рост внутри тысячи шкалу не двигает; перешагнул — следующая тысяча
  const c = await шкала([2.0,0.9,0.8,12,1.2,0]);
  chk(/1 тыс\. т/.test(c.потолок) && c.строки.find(x=>x.имя==='Спина').ширина===спина.ширина,
      '4. другая группа выросла — шкала и чужие полосы на месте',c.потолок);
  const e = await шкала([1000,2500,0,0,0,0]);
  chk(/3 тыс\. т/.test(e.потолок) && Math.abs(e.строки[1].ширина-83.33)<0.01 && /2,5 тыс\. т/.test(e.строки[1].число),
      '5. перешагнул тысячу — потолок следующая тысяча, число в тысячах тонн',e.потолок+' · '+e.строки[1].число+' '+e.строки[1].ширина+'%');
